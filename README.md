...
Wenn `local:check` einen aufgefuellten lokalen Datenbestand erkennt, meldet der Check nur einen Rehearsal-Datenhinweis: kein rotes Gate, aber auch kein sauberer Frischlauf. Der Check loescht oder archiviert lokale Daten nicht automatisch; einzelne falsche interne/synthetische Intake-Kontexte koennen nach C9 bewusst per Soft-Archiv aus aktiven Listen genommen werden.
...
- [docs/product/C9_FEHLUPLOAD_ARCHIV_LOESCH_ENTSCHEIDUNG.md](docs/product/C9_FEHLUPLOAD_ARCHIV_LOESCH_ENTSCHEIDUNG.md) dokumentiert den nach Alexander-Go umgesetzten backend-seitigen Fehlupload-Pfad: `POST /v1/intake/requests/:requestId/archive` markiert interne/synthetische Intake-Kontexte per Soft-Archiv, filtert sie aus aktiven Intake-Listen und haelt Detail-/Audit-Nachvollziehbarkeit ohne Hard-Delete, Retention-Freigabe oder echte-Daten-Go.
