...
Wenn `local:check` einen aufgefuellten lokalen Datenbestand erkennt, meldet der Check nur einen Rehearsal-Datenhinweis: kein rotes Gate, aber auch kein sauberer Frischlauf. Der Check loescht oder archiviert lokale Daten nicht automatisch; einzelne falsche interne/synthetische Intake-Kontexte koennen nach C9 in `/produktion` bewusst per Soft-Archiv aus aktiven Listen genommen werden.
