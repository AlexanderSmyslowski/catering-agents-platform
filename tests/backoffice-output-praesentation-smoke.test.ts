...
describe("backoffice production presentation smoke", () => {
  ...
  it("shows a plausible successful production result with visible assumptions and operational context", async () => {
    ...
    expect(content).not.toContain("Produktions-Draft lokal gegen den Mini-Pilot-Rahmen prüfen");
    expect(content).not.toContain("Ready oder blocked direkt im Arbeitsfluss lesen");
    expect(content).not.toContain("Status: noch kein Ergebnis");
    ...
    expect(content).not.toContain("Liste(n)");
    ...
    expect(content).toContain("Die sichtbare Liste gehört zum aktuellen Vorgang");
