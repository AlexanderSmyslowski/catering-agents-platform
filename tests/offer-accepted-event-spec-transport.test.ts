...
describe("AcceptedEventSpec production decision transport", () => {
  ...
  it("preserves the server-owned production decision and provenance through offer approval and production draft", async () => {
    ...
    const intakeApp = buildIntakeApp({
      ...
      env: { CATERING_DEFAULT_BUSINESS_ID: "local", CATERING_TRUSTED_ACTOR_SECRET: trustedSecret }
    });
  ...
  it("rejects a persisted Apply snapshot with inconsistent AcceptedEventSpec provenance without publishing artifacts", async () => {
    const rootDir = createRoot();
    const intakeStore = new IntakeStore({ rootDir });
    ...
    try {
      ...
      expectStatus(approvalResponse, 201);
      const approvedProductionSpecId = (approvalResponse.json() as {
        approvedProductionSpec: { approvedProductionSpecId: string }
      }).approvedProductionSpec.approvedProductionSpecId;
      const applyResponse = await productionApp.inject({
        method: "POST",
      ...
      expectStatus(applyResponse, 409);
      expect(applyResponse.json().message).toContain("Produktobjekte überschreiben");
      ...
      const approvedSpecPath = path.join(
        rootDir,
        "businesses",
        "local",
        "production",
        "approved-specs",
        `${encodeURIComponent(approvedProductionSpecId)}.json`
      );
      const persistedApprovedSpec = JSON.parse(readFileSync(approvedSpecPath, "utf8")) as {
        artifacts: { eventSpec: AcceptedEventSpec }
      };
      persistedApprovedSpec.artifacts.eventSpec.menuPlan[0]!.label = "Nicht freigegebene Snapshot-Mutation";
      writeFileSync(approvedSpecPath, JSON.stringify(persistedApprovedSpec, null, 2));
      const mutatedSnapshotApply = await productionApp.inject({
        method: "POST",
        url: `/v1/production/approved-specs/${approvedProductionSpecId}/apply`,
        headers: productionHeaders,
        payload: {}
      });
      expectStatus(mutatedSnapshotApply, 409);
      expect(mutatedSnapshotApply.json().errors).toContain(
        "ApprovedProductionSpec stimmt nicht mit der persistierten Freigabeevidenz überein."
      );
      expect(await productionStore.getPlan({ businessId: "local" }, artifacts.productionPlan.planId)).toBeUndefined();
    } finally {
