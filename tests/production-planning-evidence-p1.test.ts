...
async function productionFixture(
  ...
  const app = buildProductionApp({
    ...
    env: { ...appOptions.env, CATERING_DEV_AUTH: "1" }
  });
  ...
  return { app, store, auditLog, repository, discoveryService, draft, spec, caseId };
}
...
function continuationDraftForDecisionRace(
  ...
  return validateProductionDraft({
    ...
    draftId: `${sourceDraft.draftId}-${suffix}`,
    ...
    draftArtifacts: {
      ...
      eventSpec: sourceDraft.draftArtifacts.eventSpec
        ? { ...structuredClone(sourceDraft.draftArtifacts.eventSpec), specId: sourceSpecId }
        : undefined
    }
  });
}

async function persistUnprojectedContinuationWithTimelineFailure(
  store: ProductionStore,
  caseId: string,
  sourceDraft: ProductionDraft,
...
async function prepareDraftForDecision(
  app: ReturnType<typeof buildProductionApp>,
  draft: ProductionDraft,
  spec: AcceptedEventSpec,
  caseId: string
  ...
  for (const card of prepared.reviewCards) {
    const reviewResponse = await app.inject({
      ...
      headers: actorHeaders,
      payload: { decision: "fits", operatorComment: "Barrier-Review für Decision-Projektion." }
    });
...
describe("persisted production planning evidence", () => {
  ...
  it("keeps canonical Handoff provenance across revisions", async () => {
    ...
    try {
      const review = await app.inject({
        ...
        headers: actorHeaders,
        payload: { decision: "change_requested", operatorComment: "Eventdaten fuer die Revision korrigieren.", operatorCommentVisibility: "operational" }
      });
  ...
  it("linearizes initial prepare event publication before a descendant revision can commit", async () => {
    ...
    try {
      ...
      const reviewRequest = app.inject({
        ...
        headers: actorHeaders,
        payload: { decision: "change_requested", operatorComment: "Eventdaten der Revision korrigieren.", operatorCommentVisibility: "operational" }
      });
      await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    try {
      ...
      const revisedBroken = validateProductionDraft({
        ...
        reviewCards: broken.reviewCards.map((card) => ({
          ...
          operatorCommentVisibility: "operational" as const,
  ...
  it("fails closed when a review card is persisted before its case event", async () => {
    ...
    try {
      const review = await app.inject({
        ...
        url: `/v1/production/drafts/${draft.draftId}/review-cards/card-event-handoff`,
        headers: actorHeaders,
        payload: { decision: "change_requested", operatorComment: "Teilfehler pruefen.", operatorCommentVisibility: "operational" }
      });
  ...
  it("rejects applying an approved spec whose review projection is incomplete", async () => {
    ...
    try {
      ...
      expect(preparedResponse.statusCode).toBe(201);
      const preparedDraftResponse = preparedResponse.json<{ draft: ProductionDraft }>().draft;
      const preparedDraft = await store.getProductionDraft({ businessId: "local" }, preparedDraftResponse.draftId);
      expect(preparedDraft).toBeDefined();
  ...
  it("retries an approved canonical Decision idempotently without duplicate projections", async () => {
    ...
    const intakeApp = buildIntakeApp({
      ...
      env: { CATERING_DEFAULT_BUSINESS_ID: "local", CATERING_TRUSTED_ACTOR_SECRET: secret, CATERING_DEV_AUTH: "1" }
    });
  ...
  it("serializes a continuation behind delayed Decision case projection", async () => {
    const { app, store, repository, draft, spec, caseId } = await productionFixture();
    let releaseDecisionProjection!: () => void;
    const decisionProjectionBarrier = new Promise<void>((resolve) => {
      releaseDecisionProjection = resolve;
    });
    let decisionProjectionPaused = false;
    const appendEventInCollections = (store as any).appendEventInCollections.bind(store);
    const appendSpy = vi.spyOn(store as any, "appendEventInCollections").mockImplementation(async (...args: any[]) => {
      const input = args[2] as { kind?: string; text?: string };
      if (
        !decisionProjectionPaused &&
        input.kind === "review_decision" &&
        input.text === "Produktionsentwurf freigegeben."
      ) {
        decisionProjectionPaused = true;
        await decisionProjectionBarrier;
      }
      return appendEventInCollections(args[0], args[1], args[2], args[3], args[4]);
    });

    try {
      const prepared = await prepareDraftForDecision(app, draft, spec, caseId);
      ...
      await waitUntil(() => decisionProjectionPaused);
      expect(decisionProjectionPaused).toBe(true);

      const approvedDraft = await store.getProductionDraft({ businessId: "local" }, prepared.draftId);
      expect(approvedDraft?.status).toBe("approved");
      const continuation = continuationDraftForDecisionRace(
        approvedDraft!,
        `${spec.specId}-decision-continuation`,
        "decision-continuation"
      );
      let continuationSettled = false;
      const continuationPromise = store.commitDraftForCaseSource({ businessId: "local" }, {
        caseId,
        expectedSourceSpecId: spec.specId,
        nextSourceSpecId: continuation.draftArtifacts.eventSpec!.specId,
        at: continuation.createdAt,
        draftTarget: {
          kind: "production_draft",
          artifactId: approvedDraft!.draftId,
          revision: approvedDraft!.revision
        },
        draftForCaseEvent: continuation,
        commitDraft: async (scope) => {
          const inserted = await scope.insertDraft(continuation);
          const persisted = inserted === "created"
            ? continuation
            : await scope.getDraft(continuation.draftId);
          return persisted
            ? { status: "committed" as const, value: persisted }
            : { status: "conflict" as const };
        }
      });
      continuationPromise.finally(() => { continuationSettled = true; });
      await new Promise((resolve) => setTimeout(resolve, 25));
      expect(continuationSettled).toBe(false);
      releaseDecisionProjection();
      const decision = await decisionPromise;
      const continuationResult = await continuationPromise;

      expect(decision.statusCode, decision.body).toBe(201);
      expect(continuationResult.status).toBe("committed");
      const currentCase = await store.getCase({ businessId: "local" }, caseId);
      expect(currentCase?.status).toBe("open");
      expect(currentCase?.sourceSpecId).toBe(continuation.draftArtifacts.eventSpec!.specId);
      expect(currentCase?.approvedProductionSpecId).toBeUndefined();
      expect(currentCase?.currentPlanId).toBeUndefined();
      expect(currentCase?.currentPurchaseListId).toBeUndefined();
      const quantityRuntime = await app.inject({
        method: "GET",
        url: `/v1/production/cases/${caseId}/quantity-workflow`,
        headers: actorHeaders
      });
      expect(quantityRuntime.statusCode, quantityRuntime.body).toBe(200);
      expect(quantityRuntime.json<{ items: unknown[] }>().items).toHaveLength(0);
      expect(await store.listPlans({ businessId: "local" })).toHaveLength(0);
      expect(await store.listPurchaseLists({ businessId: "local" })).toHaveLength(0);
      expect(await store.listApplyManifests({ businessId: "local" })).toHaveLength(0);
      expect(await repository.list({ businessId: "local" })).toHaveLength(1);
      const approvedSpec = (await store.listApprovedProductionSpecs({ businessId: "local" }))[0];
      expect(approvedSpec).toBeDefined();
      const apply = await app.inject({
        method: "POST",
        url: `/v1/production/approved-specs/${approvedSpec!.approvedProductionSpecId}/apply`,
        headers: actorHeaders
      });
  ...
  it("rejects a continuation that wins the Decision preflight before any projection write", async () => {
    const { app, store, auditLog, repository, draft, spec, caseId } = await productionFixture();
    ...
    const originalFindCaseId = store.findCaseIdForArtifact.bind(store);
    const findCaseSpy = vi.spyOn(store, "findCaseIdForArtifact").mockImplementation(async (context, artifactId) => {
      const found = await originalFindCaseId(context, artifactId);
      if (decisionStarted && !preflightPaused && found === caseId && artifactId === preparedDraftId) {
        preflightPaused = true;
    ...
    try {
      ...
      await waitUntil(() => preflightPaused);
      expect(preflightPaused).toBe(true);

      const currentDraft = await store.getProductionDraft({ businessId: "local" }, prepared.draftId);
      const continuation = continuationDraftForDecisionRace(
        currentDraft!,
        spec.specId,
        "decision-preflight-continuation"
      );
      const continuationResult = await store.commitDraftForCaseSource({ businessId: "local" }, {
        caseId,
        expectedSourceSpecId: spec.specId,
        nextSourceSpecId: continuation.draftArtifacts.eventSpec!.specId,
        at: continuation.createdAt,
      ...
      expect(continuationResult.status).toBe("committed");
      const eventsAfterContinuation = await store.listEvents({ businessId: "local" }, caseId);
      const caseAfterContinuation = await store.getCase({ businessId: "local" }, caseId);
      const recipesAfterContinuation = await repository.list({ businessId: "local" });
      const auditsAfterContinuation = await auditLog.listRecentFor({ businessId: "local" }, 200);
      releasePreflight();
      const decision = await decisionPromise;

      expect(decision.statusCode, decision.body).toBe(409);
      expect(await repository.list({ businessId: "local" })).toEqual(recipesAfterContinuation);
      expect(await store.listApprovedProductionSpecs({ businessId: "local" })).toHaveLength(0);
      expect(await productionDecisionRepositoryFor(store).listDecisionAggregatesForDraft(
        { businessId: "local" },
        prepared.draftId
      ...
      expect(await store.listApprovalsForTarget({ businessId: "local" }, {
        kind: "production_draft",
        artifactId: prepared.draftId,
        revision: prepared.revision
      })).toHaveLength(0);
      expect(await store.listEvents({ businessId: "local" }, caseId)).toEqual(eventsAfterContinuation);
      expect(await store.getCase({ businessId: "local" }, caseId)).toEqual(caseAfterContinuation);
      expect(await auditLog.listRecentFor({ businessId: "local" }, 200)).toEqual(auditsAfterContinuation);
      expect(await store.listPlans({ businessId: "local" })).toHaveLength(0);
      expect(await store.listPurchaseLists({ businessId: "local" })).toHaveLength(0);
      expect(await store.listApplyManifests({ businessId: "local" })).toHaveLength(0);
    } finally {
      releasePreflight();
      findCaseSpy.mockRestore();
      await app.close();
    }
  });

  it("rejects an old Decision when a successor draft was persisted without its case projection", async () => {
    const { app, store, auditLog, repository, draft, spec, caseId } = await productionFixture();
    try {
      const prepared = await prepareDraftForDecision(app, draft, spec, caseId);
      const continuation = continuationDraftForDecisionRace(
        prepared,
        spec.specId,
        "decision-unprojected-successor"
      );
      await persistUnprojectedContinuationWithTimelineFailure(store, caseId, prepared, continuation);
      expect(await store.getProductionDraft({ businessId: "local" }, continuation.draftId)).toEqual(continuation);
      const eventsBefore = await store.listEvents({ businessId: "local" }, caseId);
      expect(eventsBefore.some((event) => event.artifactId === continuation.draftId)).toBe(false);
      const caseBefore = await store.getCase({ businessId: "local" }, caseId);
      const auditsBefore = await auditLog.listRecentFor({ businessId: "local" }, 200);
      const recipesBefore = await repository.list({ businessId: "local" });
      ...
      expect(await store.listApprovedProductionSpecs({ businessId: "local" })).toHaveLength(0);
      expect(await store.listEvents({ businessId: "local" }, caseId)).toEqual(eventsBefore);
      expect(await store.getCase({ businessId: "local" }, caseId)).toEqual(caseBefore);
      expect(await auditLog.listRecentFor({ businessId: "local" }, 200)).toEqual(auditsBefore);
      expect(await repository.list({ businessId: "local" })).toEqual(recipesBefore);
      expect(await store.listPlans({ businessId: "local" })).toHaveLength(0);
      expect(await store.listPurchaseLists({ businessId: "local" })).toHaveLength(0);
      expect(await store.listApplyManifests({ businessId: "local" })).toHaveLength(0);
    } finally {
  ...
  it("rejects a changed-source deterministic orphan root through the Decision case scope", async () => {
    const { app, store, auditLog, repository, draft, spec, caseId } = await productionFixture();
    let decisionScopeEntered = false;
    const originalDecisionScope = store.withCaseDecisionCriticalSection.bind(store);
    const scopeSpy = vi.spyOn(store, "withCaseDecisionCriticalSection").mockImplementation(
      async (...args: any[]) => {
        decisionScopeEntered = true;
        return (originalDecisionScope as any)(...args);
      }
    );
    try {
      const prepared = await prepareDraftForDecision(app, draft, spec, caseId);
      const fingerprint = "changed-source-root-fingerprint";
      const changedSpecId = `${spec.specId}-changed-source-orphan`;
      const deterministicRootId = `production-draft-spec-${createHash("sha256")
        .update(`local\0${caseId}\0${spec.specId}\0${fingerprint}`)
        .digest("hex")}`;
      const orphanRoot = validateProductionDraft({
        ...structuredClone(prepared),
        draftId: deterministicRootId,
        status: "pending_review",
        ...
        source: {
          ...
          inputHash: `sha256:${fingerprint}`
        },
        draftArtifacts: {
          ...structuredClone(prepared.draftArtifacts),
          eventSpec: {
            ...structuredClone(prepared.draftArtifacts.eventSpec!),
            specId: changedSpecId
          }
        }
      });
      expect(await store.insertProductionDraft({ businessId: "local" }, orphanRoot)).toBe("created");
      expect(await store.getProductionDraft({ businessId: "local" }, orphanRoot.draftId)).toEqual(orphanRoot);
      const eventsBefore = await store.listEvents({ businessId: "local" }, caseId);
      const caseBefore = await store.getCase({ businessId: "local" }, caseId);
      const auditsBefore = await auditLog.listRecentFor({ businessId: "local" }, 200);
      ...
      expect(decisionScopeEntered).toBe(true);
      expect(decision.statusCode, decision.body).toBe(409);
      expect(await productionDecisionRepositoryFor(store).listDecisionAggregatesForDraft(
        { businessId: "local" },
        prepared.draftId
  ...
  it("does not retain a Decision aggregate when its post-insert readback conflicts", async () => {
    const { app, store, auditLog, repository, draft, spec, caseId } = await productionFixture();
    const originalDecisionScope = store.withCaseDecisionCriticalSection.bind(store);
    const scopeSpy = vi.spyOn(store, "withCaseDecisionCriticalSection").mockImplementation(
      async (...args: any[]) => originalDecisionScope(
        args[0],
        args[1],
        args[2],
        async (current: any, scope: any) => {
          const originalInsert = scope.insertDecisionAggregate;
          const originalGet = scope.getDecisionAggregate;
          let inserted = false;
          scope.insertDecisionAggregate = async (aggregate: any) => {
            const result = await originalInsert(aggregate);
            inserted = result === "created";
            return result;
          };
          scope.getDecisionAggregate = async (approvalRequestId: string) => {
            const aggregate = await originalGet(approvalRequestId);
            return inserted && aggregate
              ? { ...aggregate, approval: { ...aggregate.approval, comment: "readback-conflict" } }
              : aggregate;
          };
          return args[3](current, scope);
        }
      )
    );
    try {
  ...
  it("rejects Decision before projection when the case already carries another approved spec", async () => {
    const { app, store, auditLog, repository, draft, spec, caseId } = await productionFixture();
    try {
      const prepared = await prepareDraftForDecision(app, draft, spec, caseId);
      const currentCase = await store.getCase({ businessId: "local" }, caseId);
      ...
      const conflictingCase = {
        ...currentCase!,
        approvedProductionSpecId: "approved-production-spec-from-another-decision",
        version: currentCase!.version + 1,
      ...
      expect(decision.statusCode, decision.body).toBe(409);
      expect(await productionDecisionRepositoryFor(store)
        .listDecisionAggregatesForDraft({ businessId: "local" }, prepared.draftId))
        .toHaveLength(0);
      expect(await store.listApprovalsForTarget({ businessId: "local" }, {
        kind: "production_draft",
        artifactId: prepared.draftId,
  ...
  it("clears old case projections when Decision wins before continuation", async () => {
    const { app, store, repository, draft, spec, caseId } = await productionFixture();
    try {
      const prepared = await prepareDraftForDecision(app, draft, spec, caseId);
      const decision = await app.inject({
