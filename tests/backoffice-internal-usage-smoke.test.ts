...
describe("backoffice internal usage smoke", () => {
  it("walks one realistic internal catering flow from manual intake to a usable production plan", async () => {
    ...
    expect(document.body.textContent ?? "").toContain("Spezifikation: vollständig");
    ...
    expect(document.body.textContent ?? "").toContain("Operative Bewertung im Ergebnisbereich unten.");
