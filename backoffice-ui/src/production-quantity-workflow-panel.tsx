...
type PendingPreview = {
  component: ProductionQuantityWorkflowItem;
  edit: ProductionQuantityWorkflowEdit;
  response: ProductionQuantityWorkflowPreviewResponse;
};

function numberLabel(value: number | undefined, unit: string | undefined): string {
  return value === undefined ? "–" : `${value} ${unit ?? ""}`.trim();
}

export function ProductionQuantityWorkflowPanel({ caseId }: Props) {
  ...
  const reload = async () => {
    ...
    setError(undefined);
    try {
      const result = await fetchProductionQuantityWorkflow(caseId);
      setItems(result.items);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Mengen konnten nicht geladen werden.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const previewTarget = async (item: ProductionQuantityWorkflowItem) => {
  ...
  return (
    <article className="panel secondary-panel" data-testid="production-quantity-workflow">
      <header>
        <p className="eyebrow">Mengensteuerung</p>
        <h3>Produktionsmengen prüfen und ändern</h3>
      </header>
        ...
        {items.map((item) => (
          <li key={item.componentId}>
            ...
            {item.status === "recommended" ? (
              ...
              <p className="helper-text">{item.reviewMessage ?? "Küchenentscheidung erforderlich."}</p>
            )}
            {item.canEdit && item.currentAuthority ? (
              <div>
                <label>
                  Menge pro Person ändern
                  <input
                    aria-label={`${item.label} Menge pro Person`}
                    inputMode="decimal"
                    value={draftValues[`target:${item.componentId}`] ?? String(item.currentAuthority.perUnitAmount ?? "")}
                    onChange={(event) => setDraftValues((current) => ({ ...current, [`target:${item.componentId}`]: event.target.value }))}
                  />
                </label>
                <button type="button" onClick={() => void previewTarget(item)}>Änderung prüfen</button>
              </div>
            ) : null}
            {item.purchaseRows.length > 0 ? (
              <details>
                <summary>Einkaufspositionen</summary>
                <ul className="item-list compact">
                  {item.purchaseRows.map((row) => (
                    <li key={row.rowId}>
                      <strong>{row.articleName}</strong> · {row.amount} {row.unit}
                      {row.editable ? (
                        <div>
                          <input
                            aria-label={`${row.articleName} Einkaufsmenge`}
                            inputMode="decimal"
                            value={draftValues[`purchase:${row.rowId}`] ?? String(row.amount)}
                            onChange={(event) => setDraftValues((current) => ({ ...current, [`purchase:${row.rowId}`]: event.target.value }))}
                          />
                          <button type="button" onClick={() => void previewPurchase(item, row.rowId)}>Einkaufsmenge prüfen</button>
                        </div>
                      ) : <p className="helper-text">{row.readOnlyReason}</p>}
                    </li>
                  ))}
                </ul>
              </details>
      ...
      {pending ? (
        <section aria-label="Mengenvorschau">
          <h4>Auswirkung der Änderung</h4>
          {pending.response.preview.status === "preview_ready" ? (
            <>
              <p>Neu: {numberLabel(pending.response.preview.resultingTarget?.amount, pending.response.preview.resultingTarget?.unit)} · Faktor {pending.response.preview.scaleFactor}</p>
              <ul className="item-list compact">
                {(pending.response.preview.recipeChanges ?? []).map((change) => (
                  <li key={change.ingredientId}>{change.name}: {change.baselineAmount} → {change.effectiveAmount} {change.unit}</li>
                ))}
              </ul>
