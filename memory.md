...
- P11-N3 Interner Pilot-Preflight-Runbookanker ist Doku-/Vertragstest-only umgesetzt: `docs/product/P11_N3_INTERNER_PILOT_PREFLIGHT_RUNBOOK.md` ordnet Starten, Status, UI-Routen, Reibungslog, read-only Export-/Auditbelege, kontrolliertes Stoppen und die Option-A-Zeitfenstergrenze als knappe lokale Checkliste fuer den begrenzten internen Pilot-Preflight ohne Deployment; abgesichert in `tests/p11-n3-internal-pilot-preflight-runbook-contract.test.ts`. Keine Produktlogik, UI-Aenderung, neue API/Persistenz, Deployment, Auth/OIDC, Schedule-Runtime, echten Daten oder Compliance-/DSGVO-Freigabe.
...
### 5.210 - 2026-05-24
- P11-N3 Interner Pilot-Preflight-Runbookanker ist Doku-/Vertragstest-only umgesetzt: `docs/product/P11_N3_INTERNER_PILOT_PREFLIGHT_RUNBOOK.md` ordnet den begrenzten lokalen Pilot-Preflight ohne Deployment als Checkliste Starten -> Status pruefen -> UI-Routen -> Reibungslog -> Export-/Auditbelege -> kontrolliert stoppen.
- `tests/p11-n3-internal-pilot-preflight-runbook-contract.test.ts` schuetzt die vorhandenen P11-N1/N2-, P6/P7/P9/C8- und R4-Anker, die Trennung von lokalem Preflight-Go, echter Pilotfreigabe `not assessed` und produktionsnah/echte Daten/Deployment `blocked`, sowie die Option-A-Zeitfenstergrenze. Keine Produktlogik, UI-Aenderung, API/Persistenz, Deployment, Auth/OIDC, Schedule-Runtime, echten Daten oder Compliance-/DSGVO-Freigabe.
