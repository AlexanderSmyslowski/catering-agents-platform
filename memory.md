...
- Produktionsrunner: `platform-infra/scripts/catering-target-production-update.sh`. Er prüft read-only Zielidentität, Target-Datei-Hashes, Backup-Observer, Writer-Modus, Schema-Version 3, Netz-/Porttopologie, PostgreSQL-Volume und Edge-Image; baut anschließend nur Runtime-/Web-Kandidaten, aktiviert nur die fünf Appdienste und bewahrt PostgreSQL/Edge.
...
- Fokussierter Run `35689341911` (#54) auf `23573a204…`: 43/43 Vitest und 47/47 Python-Tests grün; Build und Syntaxprüfungen grün. Die produktiven Kontrollfluss-Tests führen den echten Produktionsrunner aus und ersetzen nur externe Kommando-Grenzen.
...
### 5.163 - 2026-05-22
- B27 Hetzner-Preflight-Statusvorlage ist Doku-/Vertragstest-only umgesetzt: `docs/deployment/B27_HETZNER_PREFLIGHT_STATUS_TEMPLATE.md` macht die B26-Nachweiszeilen als ausfuellbare, nicht-sensitive Statusvorlage mit Status `go`/`blocked`/`not assessed`, nicht-sensitiver Begruendung und naechstem sicheren Schritt sichtbar.
...
### 5.166 - 2026-05-22
- B30 Hetzner-Preflight-Antwortübergabe ist Doku-/Vertragstest-only umgesetzt: `docs/deployment/B30_HETZNER_PREFLIGHT_ANSWER_HANDOFF.md` macht die B29-Operatorfragen als sichere Antwortübergabe mit Status `go`/`blocked`/`not assessed`, nicht-sensitiver Antwortnotiz und naechstem sicheren Schritt nutzbar.
- `entfernter Doku-Contract-Test` schuetzt konservative Default-Blockade, Teilantwort-Grenze, Secret-/PII-/IP-Dokumentationsgrenzen und die Nicht-Ableitung eines Deployment-Go. Kein Deployment, keine Serveraenderung, keine SSH-Verbindung, keine Secret-Erstellung, keine produktiven Configs, keine neue API/Persistenz/Migration, keine Produktlogik und keine echten Daten.
...
- PR #633 ist weiterhin ein **ungemergter vorbereitender Kandidat** fuer die Herausloesung eines anwendungsunabhaengigen `shared-edge`. Im Rehearsal bindet der Candidate ausschliesslich an `127.0.0.1:18080`; ein oeffentlicher Cutover auf 80/443 ist nicht erfolgt und wird durch diesen Stand nicht freigegeben.
- Catering wird vom neuen Edge ueber den internen TLS-Upstream angesprochen, mit dem oeffentlichen Catering-Host als TLS-SNI. Der Edge-Deploy ist hostweit serialisiert, Rollback wirkt ausschliesslich auf `shared-edge`, und App-eigene Container, Netze und Volumes bleiben ausserhalb seines Lebenszyklus.
- Der markante Zwischenstand ist zusaetzlich in `docs/agent-memory/2026-08-19-independent-edge-phase2-preparatory.md` festgehalten. Dieser Eintrag dokumentiert Vorbereitung und Rehearsal-Sicherheit; er behauptet weder Merge noch produktiven Cutover oder eine Freigabe fuer echte Daten.
...
- Exact-Head-Fachkorrektur P1 Active-Recreate-Rollbackauthority: Solange der Marker `active` trägt und das unveränderliche Baseline-Manifest Rollbackauthority bleibt, ist der Catering-Pilot für normale identitätsverändernde Platform-/Web-/Edge-/Cutover-/Recovery-Caller eingefroren. Unter beiden Locks müssen Deploy, Recreate, Restart, Rebuild, `compose up`, Source-Sync und Netzmutationen, die manifestgebundene Container-ID, `StartedAt`, Netzwerkbaseline, Mitgliedschaft, Alias oder Portbindung ändern könnten, vor Mutation fail-closed stoppen. Read-only, Hash-/Readback- und nachweislich identitätsneutrale Evidenz bleibt zulässig; ein unauthentifizierter Refresh ist verboten. Eine spätere deployfähige Phase 3 benötigt einen separat geprüften Checkpoint-/Manifestrotationsvertrag.
...
- Öffentliche Intake-, Offer-, Production- und Exportpfade verwenden denselben Session-Actor; interne Dienstaufrufe bleiben auf die vorhandenen exakten Service-/Methoden-/Pfad-Allowlisten begrenzt. Die bestehende Capability-Matrix bleibt die einzige Rechtewahrheit. Die UI lädt den Session-Kontext vor Fachdaten, hält Folgeanfragen an die aktive Sitzung gebunden und fällt bei einem späteren 401 auf den Loginzustand zurück.
- Neue sitzungsbasierte `ProductionFeedbackDraft`-Datensätze speichern bei Erstellung unveränderlich `visibility: operational | commercial` aus der bereits bestehenden Commercial-Capability. Entscheidungsänderungen bewahren den Wert; fehlende Sichtbarkeit bleibt für nichtkommerzielle Benutzer fail-closed. Audit persistiert die konkrete Session-`userId` als stabilen Actor, nicht dagegen Rolle, Business-Kontext, PIN, Hash, Kennung, Cookie oder Secret.
...
- Den explizit beauftragten Klassifikationsweg am **gleichen erhaltenen synthetischen Handofffall** geprüft. Tatsächliche Lücken in Revision 2: `menuCategory` für `kaffeepause-1` und `croissants-und-wasserservice-2`, Review-IDs `card-planning-blocker-1/-2` (kein eigener Maschinenfehlercode). Im vorhandenen Operator-Editor ausdrücklich synthetisch `classic` bzw. `vegetarian` gewählt; keine Aussage einer echten Küche.
- Echten Produktfehler reproduziert: **Antworten speichern** schrieb per Intake-PATCH und meldete Erfolg, der kanonische Handoff-Draft blieb aber unverändert; Kategorien waren nach Wiederöffnung leer. Begrenzt korrigiert: vorhandene Draft-Revisionsroute nimmt streng validierte Kategorien mit Case-/Revisionsbindung an und bewahrt den vollständigen Snapshot. Keine neue Datenhaltung oder API-Route. Unabhängige Quellfragen/Reviewpflichten bleiben erhalten; fremde Rezept-/Allergenarbeit bleibt unangetastet.
- Unabhängiger Review fand einen No-op-/Fallwechselpfad, die reale Browserabnahme anschließend veraltete getrennte Reviewpanels. Beide Fehler gezielt korrigiert und mit RED→GREEN abgesichert; unabhängiger Scope-/Qualitäts-/Risiko-Review GO. Erster enger Backend-/UI-Korridor 97 Tests, danach nur betroffene UI-Prüfungen: zuletzt 42 Tests in fünf Dateien. Typecheck und betroffener Build jeweils Exit 0, Backend bytegleich und ohne unveränderte Wiederholung. Ausgangshead `d0ce78f93a0f77a553e804214f22b07430bcb34c`, Tree `54e70dc9d33f8ebb2cf960da7540a989b3fcb774`; akzeptierter Source-/Test-Diff SHA256 `86d5888dbb639645996557e9538daf4704117e24191db1d30360f01781dd43a2`.
- Tatsächlicher Browserweg: Kategorien speichern → **Revision 3 (201)**; nach begrenzter Panelkorrektur an dieser erhaltenen Revision fortgesetzt → **Entwurf vorbereiten → Revision 4 (201)** → Reload/Fallwiederöffnung exakt gleicher Entwurf. Kategorien vorausgewählt, Fall-/Handoff-/Event-Spec-Bindung und bekannte Daten erhalten, Vorgänger 2/3 `superseded`. Keine künstliche Wiederholung des Saves nach der Panelkorrektur. Antwort-, DOM- und Timelineprüfung Exit 0, keine Konsolenfehler, Freigabe weiterhin gesperrt.
- Neu geklärt sind ausschließlich die zwei Kategorien. Danach korrekt offen: Herstellungsentscheidung für beide Komponenten und Zeitfenster; fünf Reviewkarten weiter `pending`, Plan `insufficient`, 0 Rezepte/Batches/Einkaufspositionen. Bekannte 35 Portionen je Komponente sind erhalten, keine neu validierten Produktionsmengen. Der korrigierte Handoff-Speicherweg ist auf Kategorien begrenzt; weitere Editorangaben werden ausdrücklich abgewiesen und sind noch nicht abgenommen. Menschliche Küchenprüfung bleibt offen.
- Vollständiger Abgleich der fünf Reviewpunkte, tatsächliche Kommandos/Exitcodes und Grenzen im fortgeschriebenen `docs/agent-memory/2026-09-19-gate-c-product-acceptance.md`; versionierte, nicht-sensitive Beweiszusammenfassung `docs/agent-memory/2026-09-19-classification-acceptance-evidence.json`. Eigene Dienste beendet, Daten erhalten, geschützte fremde Dateien unverändert. PR #694 bleibt Draft auf dem Produktzweig; kein Merge, Deployment, Server-/AgenturOS-Eingriff, Echtdatenlauf oder Gate-C-Gesamt-GO.


### 5.391 - 2026-09-19
- Herstellung/Zeitfenster am erhaltenen synthetischen Handofffall weitergeführt; neuer Kommentar in Draft-PR #694 vollständig berücksichtigt. Basis `d3129c5c9e2b1dc5547bb1e58dde3b7759b218d4`, Tree `964a562d3ea271713729f61735e716bfff90e0c2`; kein Reset/Angebots-Neulauf. Z1–Z3 im Zeitparser/Editor korrigiert: unveränderte strukturierte Zeitpläne bleiben erhalten, unklare Änderungen werden abgewiesen. Kanonischer Revisionsweg speichert nur explizit geänderte Kategorien/Herstellung/Zukauf/Notizen/vorhandene Rezeptauswahl/Zeitfenster.
- Unabhängiger Review fand M-R1: Änderung eines Zeitplans konnte unabhängige Prüfziele ins Leere zeigen lassen. Enger fail-closed Guard ergänzt; unveränderter Reviewer-Probe nun 422/Exit 0. Code-GO auf Source-/Test-Diff `8ad79c4da8c4de8a1d043268b0bb1c92c5f9476f19deb9e99fb4f90dad01631e`. Historisch 82 API-Tests am ersten Freeze bestanden, abschließend 40 betroffene Revisionstests und Typecheck/Build Exit 0; 55 UI-Tests und Fünf-Fälle-Zeitprüfung nach unveränderten Hashes wiederverwendet. Keine unveränderte Volltestschleife.
- Browser: Eigenproduktion Kaffeepause, Convenience-Zukauf Croissants/Wasser, Service 09:00–12:00 und ausdrückliche synthetische Notizen gemeinsam gespeichert (201). Revision 5 `production-draft-revision-fe73b9d268bebeaa20b213567133c2b95aacacaed15b1ca20b92f5b28e7fea6c` nach Reload/Wiederöffnung exakt erhalten; Kategorien, Quelle, Fall/Handoff/Spec, bekannte 35 Portionen und übrige Angaben unverändert. Vorbereitung tatsächlich 409: „Prepare benötigt exakt eine Planungs-Evidenz je Produktionskomponente.“ Keine zusätzliche Revision, kein neuer Plan, keine Rezepte/Einkaufsliste, kein Review bestätigt.
- Nächste tatsächliche Voraussetzung: für `kaffeepause-1` fehlen Bibliotheksrezept, Mengenentscheidung und menschliche Event-Rezeptprüfung als fall-/revisionsgebundene Planungs-Evidenz. Backend-Gate vorhanden, passender Backoffice-Aufrufer fehlt; Mengenworkflow des unfreigegebenen Falls liefert leer. Keine neue Rezept-/Mengen-/Freigabeoberfläche abgeleitet und keine Küchenprüfung fingiert. Vollständige Prepare-Abnahme weiterhin blockiert; kein Gate-C-Gesamt-GO.
- Nachweise/Einordnung: `docs/agent-memory/2026-09-19-gate-c-product-acceptance.md` und `docs/agent-memory/2026-09-19-manufacturing-acceptance-evidence.json`; 44 Browserkommandos Exit 0, Payload-/DOM-/Revisionsvergleich Exit 0. Genau ein erwarteter Netzwerkfehler aus Prepare409. Eigene Dienste/Browser beendet, Datenroot und fremde Rezept-/Allergenarbeit erhalten. Nur Produktzweig, kein Merge/Deployment/Server-/AgenturOS-Eingriff.


### 5.392 - 2026-09-19

- PR-#694-Kommentar 5743033256 am bestehenden synthetischen Fall vollständig umgesetzt. Das Backoffice verbindet jetzt den vorhandenen Rezeptupload/-bestand, kanonische Rezeptzuordnungsrevision, konkrete Mengenentscheidung, vier ausdrückliche Event-Rezeptbestätigungen und die vorhandene Planning-Evidence-Route. Keine neue Route oder Datenhaltung. Zukauf bleibt von Eigenproduktions-Evidenz ausgenommen; Fall-/Komponenten-/Draft-/Revisions-/Rezept-/Snapshot-/Actor-Bindungen sowie stale, duplicate und malformed Zugriffe bleiben fail-closed.
- Unabhängiger Review hielt zwei Important-Befunde an: Objekt-/Arraywerte in Mengen-Enums und veraltete Retry-Nutzlasten nach endgültigem 409/422. Beide TDD-korrigiert; abschließender Code- und Build-Delta-Review GO ohne offene Befunde. Betroffener Gesamtkorridor: 15 Testdateien, 286/286 Tests, Typecheck, Build und Diffcheck jeweils Exit 0. Geprüfter Zehn-Dateien-Inhaltsfingerabdruck `1624621f9f442f4fcfc30e75e4127b511b0e09a230b19645a20b795a17c10f2e`; Basis `24519660988db8970a7c84db58a39c0e979ba557`, Tree `79ef97b995e0aa8b0761ace4707903d63ae2fea0`.
- Browserkette im isolierten Abzug des erhaltenen Falls: ausdrücklich synthetische Rezept-Fixture hochgeladen (201, weiter `review_required`) → Rezeptzuordnung Revision 5→6 (201) → eine Portion pro Person für 35 Gäste, Rolle `snack`, vier technische Testbestätigungen und exakte Planungs-Evidenz für Revision 6 gespeichert (201) → Prepare Revision 6→7 (201) → Reload/Wiederöffnung desselben Falls. Fall, Handoff, Event-Spec, Kategorien, Herstellung, Zukauf und 09:00–12:00 erhalten. Ohne vertrauenswürdigen Operator 403; Replay gegen supersedierte Revision 6 409; Browserkonsole ohne Fehler.
- Revision 7 enthält tatsächlich einen Rezept-Snapshot, einen Batch über 35 Portionen/vier Chargen, zwei Küchenblätter und fünf Einkaufspositionen. Rezeptlineage: 350 g Kaffeebohnen, 70 g Tee, 3,5 l Haferdrink; Zukauflinie: Croissants und Wasser je 35 Portionen. Vier Reviewkarten für Event, Plan, Einkauf und Rezept/Skalierung bleiben `pending`; `approvedProductionSpecs=[]`, kein angewendeter Plan und keine vollständige Exportabnahme. Evidenz von Revision 6 wird in Revision 7 nur als Vorgängerherkunft gezeigt, nicht als neue Prüfung übertragen.
- Die vier Bestätigungen stammen von einem synthetischen Testakteur und ersetzen keine reale menschliche Küchenprüfung. Rezept bleibt ungeprüfter Kandidat; Allergene, fachliche Methode/Ausstattung, Warmhaltung/Regeneration sowie Zukaufsspezifikation, Gebinde, Lieferant und reale Einkaufsdeckung bleiben offen. Nachweise: `docs/agent-memory/2026-09-19-gate-c-product-acceptance.md` und `docs/agent-memory/2026-09-19-recipe-planning-evidence-acceptance.json`. Eigene Dienste beendet, Alternativports frei, ursprünglicher Datenroot und fremde Arbeit erhalten. Kein Merge, Deployment, Server-/AgenturOS-Eingriff oder Gate-C-Gesamt-GO.

### 5.393 - 2026-09-19

- Auf der erhaltenen isolierten D7-Kopie `/tmp/catering-recipe-continuation-4ANJAV` wurde der zusammenhängende Zukauf-/Review-/Übernahme-/Exportweg abgeschlossen. Der kanonische Operatorweg speicherte die ausdrücklich synthetischen Mengen **Croissants 1 Stück je Person → 35 Stück** und **Wasser 0,5 l je Person → 17,5 l** als Revision 8; nach korrekt invalidierter und frisch gebundener Planungs-Evidenz entstand Revision 9. Kategorien, Herstellungsarten, Rezept, Service 09:00–12:00 sowie Fall-/Handoff-/Event-Spec-Bindung blieben erhalten.
- Alle vier D9-Reviewkarten wurden durch den synthetischen Testakteur einzeln anhand ihrer eingefrorenen Inhalte entschieden. Die technische Freigabe und Apply erzeugten genau einen Plan, eine Einkaufsliste und den bestehenden Rezept-Snapshot; identischer Apply-Retry blieb idempotent. Der Plan hält 35 Portionen und vier Chargen getrennt, Zutaten bleiben 350 g Kaffeebohnen, 70 g Tee und 3,5 l Haferdrink. Die Einkaufsliste enthält genau fünf hergeleitete, eindeutig zugeordnete Zeilen einschließlich 35 Stück Croissants und 17,5 l Wasser.
