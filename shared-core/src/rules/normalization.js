...
function detectServiceForm(text, eventType) {
...
function parseAttendees(text) {
    const directMatch = text.match(/\b(?:für|fuer)\s+(\d{1,4})\s*(?:gäste|gaeste|teilnehmer|personen|people)?\b/i) ??
...
function inferMenuCategoryFromText(line) {
...
function requiresExplicitCategoryDecision(line) {
    const normalized = sanitizeMenuLine(line).toLowerCase();
    return /^(brot(\s*&\s*baguette)?|baguette|br[öo]tchen|brotkorb|butter|dips?|saucen?)$/i.test(normalized);
}
...
function looksLikeMenuHeading(line) {
    return /^(quick lunch|lunch|traditionell|vegan|vegetarisch|dessert|buffet|menü|menu)(\s*\|\s*\d+%?)?$/i.test(line);
}
function isMenuNoise(line) {
    return /(?:uhr|gesamt|kosten|position|beschreibung|personalkosten|lieferung|transport|aufbau|abbau|umbau|rücklauf|personaleinsatz|hall of fame|hauptspeisenteller|stehttische|stehtische|geschirr|tischdecken|reinigungskosten|stunden|stunde)/i.test(line);
}
function extractStructuredMenuSection(text) {
    ...
    for (const line of lines) {
        ...
        const lineCategory = explicitCategory ??
...
function extractMenuItems(text, fallbackKeywords) {
    ...
    const detected = lines.flatMap((line) => {
        ...
        return /(buffet|salat|suppe|kaffee|croissant|dessert|fingerfood|wein|snack|menü|baguette|brot|kuchen|curry)/i.test(line) && !isMenuNoise(line)
            ? [
                {
                    label: line,
...
function inferInfrastructure(eventType, desiredInfrastructure) {
    const defaults = eventTypeDefaults[eventType]?.infrastructure ?? [];
    const base = defaults.map((code) => ({
        code,
        label: infrastructureCatalog[code] ?? code,
        quantity: 1,
        derived: true
    }));
...
export function normalizeEventRequestToSpec(request, opts) {
    ...
    const mergeUniqueStrings = (primary, secondary) => {
