...
export const llmReadinessRunResultVersion = "llm-readiness-run-result-v0";

export const llmReadinessRunResultStatuses = ["completed", "rejected"];

function isRecord(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function hasAllowedStatus(value) {
...
function validateAuditConsistency(request, response, auditRecord) {
  const errors = [];
...
export function validateLlmReadinessRunResult(candidate) {
  const errors = [];
...
export function createLlmReadinessRunResult(buildRequest) {
  const errors = [];
  ...
  errors.push(...validateAuditConsistency(buildRequest.request, buildRequest.response, buildRequest.auditRecord));
