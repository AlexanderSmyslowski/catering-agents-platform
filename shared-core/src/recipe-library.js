...
const ignoredImportedRecipePattern = /\b(produktionsblatt|abschiebeplan|standing reception|paschtu|bleche[- ]?gn[- ]?plan)\b/i;
...
function deriveArchetypeTokens(label) {
...
function deriveCompoundStemTokens(token) {
    const stems = [];
...
function leadSpecificPrimaryToken(label) {
    return specificPrimaryFocusTokens(label)[0];
}
function expandQuerySpecificTokens(tokens) {
    const expanded = new Set();
...
function ingredientGroupFor(name) {
    const normalized = name.toLowerCase();
    const match = Object.entries(ingredientGroupHints).find(([keyword]) => normalized.includes(keyword));
    return match?.[1] ?? "misc";
}
...
function recipeNameFromText(text, filename, override) {
    if (override?.trim()) {
...
function servingsFromText(text) {
    const match = text.match(/(\d{1,3})\s*(portionen|portions|servings|personen|gaeste|gäste|people)/i) ??
...
export function parseUploadedRecipeText(input) {
    ...
    const ingredientsSource = sections.ingredients.length > 0 ? sections.ingredients : sections.notes.filter(isIngredientLine);
    const ingredients = ingredientsSource
        .map((line, index) => parseIngredientLine(line, index))
        .filter((item) => Boolean(item));
    const stepsSource = sections.steps.length > 0
        ? sections.steps
        : sections.notes.filter((line) => !isIngredientLine(line)).slice(0, 12);
    const steps = (stepsSource.length > 0
        ? stepsSource
        : ["Bitte die hochgeladenen Zubereitungshinweise vor der Produktion pruefen."]).map((instruction, index) => ({
        index: index + 1,
    ...
    const coverageScore = Number(Math.min(1, (ingredients.length > 0 ? 0.45 : 0) +
        (steps.length > 0 ? 0.35 : 0) +
        (text.length > 120 ? 0.1 : 0) +
        (sections.ingredients.length > 0 && sections.steps.length > 0 ? 0.1 : 0)).toFixed(2));
    return validateRecipe({
        ...
        source: {
            ...
            licenseNote: "Menschlich hochgeladene interne Rezeptquelle. Automatisch extrahierte Zutaten, Allergene und Diet-Tags erfordern Review vor operativer Nutzung.",
...
export class RecipeLibrary {
    ...
    constructor(seed = internalRecipes, options) {
        this.recipes = createPersistentCollection({
    ...
    async findCandidates(label) {
        ...
        return (await this.recipes.list())
            ...
            const archetypeOverlap = requiredArchetypes.filter((token) => [...rightTokens].some((candidateToken) => tokensRoughlyMatch(token, candidateToken))).length;
            const specificOverlap = specificTokens.filter((token) => [...rightTokens].some((candidateToken) => tokensRoughlyMatch(token, candidateToken))).length;
            ...
            const overlap = [...leftTokens].filter((token) => [...rightTokens].some((candidateToken) => tokensRoughlyMatch(token, candidateToken))).length;
            const primaryOverlap = primaryTokens.filter((token) => [...rightTokens].some((candidateToken) => tokensRoughlyMatch(token, candidateToken))).length;
            const focusOverlap = focusTokens.filter((token) => recipeSpecificTokens.some((candidateToken) => tokensSpecificallyMatch(token, candidateToken))).length;
            if (focusTokens.length > 0 && focusOverlap === 0) {
                return { recipe, score: 0 };
            }
            const leadSpecificToken = leadSpecificPrimaryToken(label.label);
            const leadNameBoost = leadSpecificToken &&
                recipeNameSpecificTokens.some((candidateToken) => tokensSpecificallyMatch(leadSpecificToken, candidateToken))
                ? 0.3
                : 0;
            const leadingBoost = normalizeSearchText(recipe.name).includes(primaryTokens[0] ?? "")
                ...
                : normalizeSearchText(recipe.source.reference).includes(primaryTokens[0] ?? "")
                    ? 0.1
                    : 0;
            const phraseBoost = primaryTokens.length >= 2 &&
                normalizeSearchText(recipeSearchText).includes(primaryTokens.join(" "))
                ? 0.15
                : 0;
            const broadMatchPenalty = primaryOverlap === 0 && !normalizeSearchText(recipeSearchText).includes(normalizedLabel.split(" ")[0] ?? "")
                ? 0.1
                : 0;
    ...
    async reviewRecipe(recipeId, input) {
