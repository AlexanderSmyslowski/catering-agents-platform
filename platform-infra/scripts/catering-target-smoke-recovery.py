...
'''
        ...
        while time.monotonic() < deadline:
            ...
            if fd in readable:
                ...
                if chunk:
                    ...
                    if not login_ready and b"TARGET_SMOKE_LOGIN_READY" in captured:
                        login_ready = True
                    if login_ready and not login_sent:
                        os.write(fd, login_code.encode("utf-8") + b"\n")
                        login_sent = True
                    if not pin_ready and b"TARGET_SMOKE_PIN_READY" in captured:
                        pin_ready = True
                    if pin_ready and not pin_sent:
                        os.write(fd, pin.encode("ascii") + b"\n")
                        pin_sent = True
                    if b"TARGET_SMOKE_BOOTSTRAP_OK" in captured:
                        final_ok = True
                else:
    ...
    print(
        ...
        f"remote_fail={str(fail_marker).lower()}"
    )
