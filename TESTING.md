...
- `tests/upload-security.test.ts`
  ...
  - P3-B37 Upload-Grenzen als Beta-Risiko: Intake-Dokumentuploads: maximal 25 MB pro Datei und bis zu 3 Dateien pro Multipart-Request; Rezeptuploads in Angebot und Produktion: maximal 5 MB und genau eine Datei pro Upload. Erlaubt bleibt nur der vorhandene Dokumentkorridor PDF/TXT/MD/EML/Pages mit passender MIME-/Extension-Kombination. Produktionsnahe Verarbeitung echter oder beliebiger Uploads bleibt ohne Sandbox/Worker/AV-Gate `blocked`; Warnungen bleiben sichere Dokumentprüfungsmarker ohne Rohtext- oder Vollhash-Spiegelung.
...
Diese Tests pruefen den Kernpfad von Intake/AcceptedEventSpec ueber Produktionsplanung, Rezept-/Kitchen-Sheet-Erzeugung, Einkaufsliste, Fallbacks, produktionsnahe Sprache, die read-only Conversation-Projection, den PA5-Nachvollziehbarkeitskorridor Upload-Provenance -> Conversation-Quellenanker -> Produktionsoutput/Exportdarstellung, die PA6-Doku-Grenze fuer interne Beta-/Abnahme-Readiness, die PA7-Entscheidungs-ADR fuer AuthN/AuthZ + read-path Auth, den PA8-Guard-Korridor fuer sensible read-only Detail-/Listen-/Exportpfade, den PA9-Proxy-/Deployment-Readiness-Anker fuer Header-Stripping/Trusted-Header-Injektion/Health-Grenzen, die B8 AuthN/AuthZ/read-path Auth Entscheidungsgrenze fuer den naechsten minimalen produktionsnahen Auth-Korridor, den B9 Proxy/IAP-AuthN-Preflight-Vertrag fuer Header-Stripping am Proxy-/IAP-Rand, kontrollierte Trusted-Header-Injektion, serverseitiges Trusted Secret, nicht direkt exponierte Services, nicht-sensitive Health-Endpunkte und interne read-only Arbeitsbelege hinter Trusted-Actor-/Proxy-Kontext, das B10 Pilot-Preflight-Runbook als ausfuellbaren Vor-Pilot-Korridor mit Zielumgebung, Betreiber, Proxy-/IAP-Rahmen, Ergebniszustaenden `go`/`blocked`/`not assessed` und separaten PII-/Retention-/Backup-/Sandbox-/AV-Gates, den schmalen Upload-/PDF-Haertungskorridor fuer Groessenlimits und MIME-/Extension-Allowlist inklusive C6-Workbench-Sichtbarkeit kontrollierter Upload-Fehler und sicherer Dokumentwarnungen, den PA10-PA14 DocumentIngestion-Korridor, die PA15-Entscheidungsvorlage fuer die naechste ProductionAgent-v1-Faehigkeit sowie das PA16/PA17-Rueckfragenmodell als engen Clarification-Slice inklusive Priorisierung, Deduplizierung und sicherer Labels, das PA18-Antwortverarbeitungs-Gate als Doku-/Security-Anker ohne Runtime-Antwortannahme, den PA19-Typanker fuer spaetere Clarification-Antworten ohne Runtime-, API- oder Persistenzannahme, die PA20-Entscheidungsvorlage fuer ein spaeteres Answer-Datenmodell-/Migrationsgate, den PA21-Modellanker fuer `ProductionClarificationAnswer` ohne Runtime-/Persistenz-/API-Ausweitung, das PA22-Speicher-/Anzeige-Gate fuer spaetere kurze Freitextantworten ohne PA22-Runtime-Implementierung, PA23 als ersten engen Runtime-Speicher-/Anzeige-Slice fuer `submitted`-`shortText`-Antworten in bestehender Persistenz-/Projection-Grenze, PA24 als Spec-/Session-Bindungsanker fuer Clarification-Antworten auf Basis bestehender `specId`/`production-session-${specId}`-Kontexte, PA25 als read-only Statusanker fuer beantwortete Rueckfragen in derselben Projection, PA26 als LLM-Readiness-Vertrag ohne Provider/Runtime/Schreibwirkung, PA27 als synthetische LLM-Readiness-Eval-Fixture-Grenze, PA28 als schema-only Draft-Registry zwischen Model-IO und Fixtures, PA29 als Input-Validation-Grenze fuer providerlose Model-Input-Kandidaten, PA30 als zentrale Eval-Fixture-Validation-Grenze, PA31 als SourceRef-Allowlist-Grenze, PA32 als StructuredCandidate-Scalar-Grenze, PA33 als ExpectedOutput-SourceRef-Vertragsgrenze, PA34 als SourceRef-Identity-Parity-Grenze, PA35 als Draft-Registry-Coverage-Grenze, PA36 als providerlose Eval-Harness-Grenze, PA37 als schema-only Prompt-/Schema-Registry-Grenze, PA38 als fixture-only ProviderAdapter-Grenze, PA39 als providerlosen AgentAudit-Grenze, PA40 als providerlosen synthetic-only Run-Result-Grenze, PA41 als entscheidungsreifen Provider-/Daten-/Runtime-Rahmen sowie C11 als aktuelle 10/10-Gap-Audit-Grenze zwischen belegtem internem 9/10-Rehearsal, autonomer LLM-Readiness-Vorbereitung ohne Provider und blockierten echten Gates.

PA56 LLM Prompt-/Response-Retention- und Evidence-Entscheidungsrahmen ist als Doku-/Vertragsanker codiert: `docs/architecture/PA56_LLM_RETENTION_EVIDENCE_DECISION_FRAME.md` zieht nach PA54 und PA55 die naechste Schwesterfrage scharf: welcher enge Prompt-/Response-Retention- und Evidence-Rahmen fuer spaetere providerfaehige Draft-Laeufe ueberhaupt denkbar ist, ohne Raw-Logging, allgemeine Artefaktspiegelung, Backup-Retention oder Runtime-Ausweitung still zu oeffnen. `entfernter Doku-Contract-Test` schuetzt die Grenzen gegen Deployment, neue APIs, Persistenz, Migration, echte Daten, Backup-Aktivierung und Produktschreibwirkung sowie die Empfehlung fuer strukturierte Default-Nachweise plus eng begrenzte lokale redigierte Review-Ausschnitte.

PA57 LLM Deployment-/Zielumgebungs-Entscheidungsrahmen ist als Doku-/Vertragsanker codiert: `docs/architecture/PA57_LLM_DEPLOYMENT_TARGET_ENVIRONMENT_DECISION_FRAME.md` zieht nach PA56 die naechste Schwesterfrage scharf: unter welchem Deployment-/Zielumgebungs-Kontext ein spaeterer nicht-lokaler providerfaehiger Draft-Pfad ueberhaupt denkbar ist, ohne B25-B37 oder PA9 still in Infrastruktur-, Server- oder Runtime-Arbeit zu kippen. `entfernter Doku-Contract-Test` schuetzt die Grenzen gegen Deployment, Serveraenderung, SSH, Secret-Erstellung, neue APIs, Persistenz, Migration, echte Daten und Produktschreibwirkung sowie die Empfehlung fuer einen strikt deployment-gated nicht-lokalen Rahmen.

PA58 LLM Human-Approval-/Operator-Handover-Entscheidungsrahmen ist als Doku-/Vertragsanker codiert: `docs/architecture/PA58_LLM_HUMAN_APPROVAL_OPERATOR_HANDOVER_DECISION_FRAME.md` zieht nach PA57 die naechste Schwesterfrage scharf: wie Human Approval, Operator-Handover und manuelle Draft-Uebernahme fuer spaetere nicht-lokale providerfaehige Draft-Pfade aussehen muessen, ohne eine neue Approval-Runtime oder stilles Self-Approval zu normalisieren. `entfernter Doku-Contract-Test` schuetzt die Grenzen gegen Deployment, neue Approval-Engine, neue APIs, Persistenz, Migration, echte Daten und Produktschreibwirkung sowie die Empfehlung fuer strikt human-in-the-loop gebliebene Handover-Schritte.

PA59 LLM Tool-/Write-Effect-Entscheidungsrahmen ist als Doku-/Vertragsanker codiert: `docs/architecture/PA59_LLM_TOOL_WRITE_EFFECT_DECISION_FRAME.md` zieht nach PA58 die naechste Schwesterfrage scharf: welche Tool-Klassen ein spaeterer providerfaehiger Draft-Pfad ueberhaupt sehen darf und wo die harte Grenze gegen Write-Effects, Tool-Orchestrierung und produktwirksame Uebergaben bleibt. `entfernter Doku-Contract-Test` schuetzt die Grenzen gegen Deployment, Tool-Orchestrierung, neue APIs, Persistenz, Migration, echte Daten und Produktschreibwirkung sowie die Empfehlung fuer strikt read-/draft-only gebliebene Tool-Reichweite.

PA60 LLM Runtime-/ConversationSession-Entscheidungsrahmen ist als Doku-/Vertragsanker codiert: `docs/architecture/PA60_LLM_RUNTIME_CONVERSATION_SESSION_DECISION_FRAME.md` zieht nach PA59 die naechste Schwesterfrage scharf: ob ein spaeterer providerfaehiger Draft-Pfad ueberhaupt eine echte Runtime-`ConversationSession` brauchen darf oder fuer den ersten freigegebenen Korridor bewusst projektionsbasiert bleiben muss. `entfernter Doku-Contract-Test` schuetzt die Grenzen gegen Deployment, neue Runtime-Conversation, neue APIs, Persistenz, Migration, echte Daten und Produktschreibwirkung sowie die Empfehlung, den ersten freigegebenen Draft-Pfad ohne neue Session-Runtime zu halten.

PA61 LLM Dokument-/Upload-Quellen-Sicherheitsrahmen ist als Doku-/Vertragsanker codiert: `docs/architecture/PA61_LLM_DOCUMENT_UPLOAD_SOURCE_SAFETY_DECISION_FRAME.md` zieht nach PA60 die naechste Schwesterfrage scharf: ob ein spaeterer providerfaehiger Draft-Pfad direkte Uploads, Rohdokumente oder Rohtext-nahe Dokumentquellen jemals sehen darf oder bewusst auf reduzierte, nicht-dateinahe Arbeitsausschnitte begrenzt bleiben muss. `entfernter Doku-Contract-Test` schuetzt die Grenzen gegen Deployment, Sandbox-/Worker-/AV-Implementierung, neue APIs, Persistenz, Migration, echte Daten und Produktschreibwirkung sowie die Empfehlung, direkte Uploads und Rohdokumente aus dem Providerpfad herauszuhalten.
...
PA41 LLM Provider-/Daten-/Runtime-Entscheidungsrahmen ist als Doku-/Vertragstestanker codiert: `docs/architecture/PA41_LLM_PROVIDER_DATA_RUNTIME_DECISION_FRAME.md` verdichtet den naechsten echten LLM-Gate-Schritt nach PA40 in eine klare Alexander-Vorlage mit Optionen, Empfehlung, Konsequenz und sicherem Default. `entfernter Doku-Contract-Test` schuetzt, dass die providerlose Kette bis PA40 explizit bleibt, Option B als minimaler synthetic-only Provider-Slice klar benannt ist und breitere Runtime-/Write-/Echte-Daten-Schritte weiter blockiert bleiben.

PA51 LLM Operator-/Kosten-/Approval-Entscheidungsrahmen ist als Doku-/Vertragstestanker codiert: `docs/architecture/PA51_LLM_OPERATOR_COST_APPROVAL_DECISION_FRAME.md` zieht die naechste Management-Vorlage nach dem bereits umgesetzten lokalen synthetic-live Korridor: wer ihn lokal ausfuehren darf, welches Kostenlimit gilt, welche Modellgrenzen erlaubt sind und wie Human Approval als Betriebsregel sichtbar bleibt. `entfernter Doku-Contract-Test` schuetzt, dass PA42-PA50 als vorhandener lokaler Korridor anerkannt werden, Option B den kleinsten lokalen Operatorrahmen empfiehlt und breitere nicht-lokale oder produktwirksame Runtime-Schritte weiter blockiert bleiben.

PA52 Synthetic-Live Local Operator Runbook ist als Doku-/Vertragstestanker codiert: `docs/architecture/PA52_SYNTHETIC_LIVE_LOCAL_OPERATOR_RUNBOOK.md` operationalisiert PA51 als kleinsten lokalen Bedienrahmen fuer den vorhandenen `synthetic_live`-Korridor: nur benannte interne Operatoren, nur lokale Ausfuehrung, nur lokale Secrets ausserhalb des Repos, nur synthetische Fixtures, expliziter Kostenrahmen, kein Raw Prompt-/Response-Logging und Human Approval vor jeder manuellen Uebernahme. `entfernter Doku-Contract-Test` schuetzt diesen Runbook-Rahmen, ohne neue Runtime, neue APIs, Persistenz, Deployment, echte Daten oder Schreibwirkung einzufuehren.

PA62 Synthetic-Live Mini-Pilot Policy ist als kleiner Runtime-/Vertragstestanker codiert: `docs/architecture/PA62_SYNTHETIC_LIVE_MINI_PILOT_POLICY.md` codiert den beschlossenen Option-2-Mini-Pilot als zusätzliche Preflight-Policy-Schicht ueber dem vorhandenen `synthetic_live`-Korridor. `tests/pa62-synthetic-live-mini-pilot-policy.test.ts` schuetzt, dass ein Lauf nur dann als mini-pilot-ready gilt, wenn der enge Nutzer-, Daten-, Draft- und Human-Approval-Rahmen explizit markiert ist, ohne neue Provider-Runtime, echte Daten, Deployment, Persistenz oder Produktschreibwirkung einzufuehren.

PA63 Synthetic-Live Mini-Pilot Probe Guard ist als kleiner Runtime-/Vertragstestanker codiert: `docs/architecture/PA63_SYNTHETIC_LIVE_MINI_PILOT_PROBE_GUARD.md` legt einen dedizierten lokalen Probe-Entry ueber den vorhandenen `synthetic_live`-Korridor, der nur dann ausfuehrt, wenn der Preflight auch den PA62-Mini-Pilot-Rahmen bestaetigt. `tests/pa63-synthetic-live-mini-pilot-probe-guard.test.ts` schuetzt das neue CLI-Flag, den dedizierten npm-Entry und den harten Abbruch bei fehlender Mini-Pilot-Freigabe, ohne neue Provider-Runtime, API, Persistenz, Deployment oder Schreibwirkung einzufuehren.

PA64 Synthetic-Live Mini-Pilot Check Entry ist als kleiner Runtime-/Vertragstestanker codiert: `docs/architecture/PA64_SYNTHETIC_LIVE_MINI_PILOT_CHECK_ENTRY.md` buendelt den vorhandenen Preflight und den PA63-guarded Probe-Lauf zu einem einzigen lokalen Mini-Pilot-Einstieg. `tests/pa64-synthetic-live-mini-pilot-check-entry.test.ts` schuetzt den neuen npm-Entry, das gemeinsame JSON-Ergebnis und den harten Fehler bei fehlendem PA62-Rahmen, Probe-Fehler oder Eval-Drift, ohne neue Provider-Runtime, API, Persistenz, Deployment oder Schreibwirkung einzufuehren.

PA65 Synthetic-Live Mini-Pilot Summary Signal ist als kleiner Runtime-/Vertragstestanker codiert: `docs/architecture/PA65_SYNTHETIC_LIVE_MINI_PILOT_SUMMARY_SIGNAL.md` ergaenzt den vorhandenen Mini-Pilot-Check um ein lesbares `summary`-Signal mit `status`, `reason` und `nextStep`. `tests/pa65-synthetic-live-mini-pilot-summary-signal.test.ts` schuetzt, dass der Check bei Erfolg klar `ready` meldet und bei fehlendem Rahmen, Probe-Fehler oder Eval-Drift als `blocked` mit passendem Hinweis endet, ohne neue Provider-Runtime, API, Persistenz, Deployment oder Schreibwirkung einzufuehren.

PA53 Synthetic-Live Preflight Policy Hints ist als kleiner Runtime-/Vertragstestanker codiert: `docs/architecture/PA53_SYNTHETIC_LIVE_PREFLIGHT_POLICY_HINTS.md` erweitert den bestehenden PA49-Preflight um weiche lokale Policy-Hinweise fuer den in PA52 beschriebenen Bedienrahmen: benannter interner Operator, explizite Budgetnotiz, Human Approval bleibt Pflicht und Raw Prompt-/Response-Logging bleibt verboten. `tests/pa53-synthetic-live-preflight-policy-hints.test.ts` schuetzt, dass diese Signale als Warnings und Readiness-Hinweise sichtbar werden, ohne neue harte Deployment-, Daten-, Persistenz- oder Runtime-Gates einzufuehren.

PA54 LLM Daten-/PII-Entscheidungsrahmen ist als Doku-/Vertragstestanker codiert: `docs/architecture/PA54_LLM_DATA_PII_DECISION_FRAME.md` zieht die naechste Datenfrage nach PA53 scharf: ob ein spaeterer providerfaehiger Draft-Pfad ausschliesslich synthetic/demo bleibt oder hoechstens nachweisbar anonymisierte, reduzierte Draft-Inputs sehen duerfte. `entfernter Doku-Contract-Test` schuetzt, dass PA54 B13, P11-N2 und P12-N2 als fuehrende Daten-/PII-Gates nutzt und pseudonymisierte oder echte Inputs weiter blockiert, ohne neue Runtime, Persistenz, Deployment oder Datenverarbeitung einzufuehren.

PA55 LLM Trusted-Operator-/Auth-Entscheidungsrahmen ist als Doku-/Vertragstestanker codiert: `docs/architecture/PA55_LLM_TRUSTED_OPERATOR_AUTH_DECISION_FRAME.md` zieht die Schwesterfrage zu PA54 scharf: unter welchem Trusted-Operator-/Auth-Kontext ein spaeterer providerfaehiger Draft-Pfad ueberhaupt betrieben werden duerfte. `entfernter Doku-Contract-Test` schuetzt, dass PA55 B8, B9 und PA9 als fuehrende Auth-/Proxy-Gates nutzt, freie Client-Header oder lokales `x-actor-name` nicht zu belastbarer LLM-Operatoridentitaet aufwertet und keine Login-/Session-/OIDC-/Runtime-Ausweitung einfuehrt.

P5-B49 Beta-Durchlauf Ist-Karte ist als Doku-/Vertragstestanker codiert: `docs/product/P5_BETA_DURCHLAUF_IST_KARTE.md` kartiert den vorhandenen internen Nutzerweg `Start -> Angebot -> Produktion -> Exporte/Audit` und trennt intern nutzbar, nur dokumentiert / nur intern abnahmefaehig, blockiert und schon testbar. `entfernter Doku-Contract-Test` schuetzt die Auffindbarkeit aus README, TESTING und C8 sowie die Grenzen: kein Deployment, keine SSH-Verbindung, keine echten Daten, keine neue Persistenz, kein OAuth/Login/OIDC, keine automatische Spec-Korrektur und keine Rezept-/Allergenautomatik.
...
- Der Preflight spiegelt zusaetzlich weiche lokale Policy-Hinweise fuer `CATERING_SYNTHETIC_LLM_OPERATOR_NAME` und `CATERING_SYNTHETIC_LLM_BUDGET_NOTE`; fehlende Werte sind kein harter Fehler, markieren den Lauf aber als nicht voll policy-ready.
...
- `npm run llm:synthetic-live:probe:mini-pilot` ist derselbe Probe-Lauf mit hartem Mini-Pilot-Guard und bricht ab, wenn der vorhandene Preflight den PA62-Rahmen nicht vollstaendig bestaetigt.
- `npm run llm:synthetic-live:check:mini-pilot` ist der kompakte Operator-Einstieg fuer den freigegebenen engen Mini-Pilot und liefert Preflight plus guarded Probe als gemeinsames JSON-Ergebnis.
- Das bindende lokale Bedien-Runbook dazu ist `docs/architecture/PA52_SYNTHETIC_LIVE_LOCAL_OPERATOR_RUNBOOK.md`: nur benannte interne Operatoren, nur lokale Secrets ausserhalb des Repos, kein Raw Prompt-/Response-Logging, expliziter Kostenrahmen und Human Approval vor jeder manuellen Weiterverwendung.
