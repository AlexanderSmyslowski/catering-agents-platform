...
P9-N3 Rehearsal-Reibung-zu-Entscheidung ergaenzt fuer die Nachbereitung den kopierbaren Ergebnisanker `go` / `fix` / `blocked` / `decision needed`: `go` nur fuer eine widerspruchsfreie lokale synthetische Rehearsal-Kette, `fix` fuer kleine beobachtete und lokal reproduzierbare Doku-/Copy-/Smoke-Reibung, `blocked` fuer Stop-Gates oder rote lokale Gates und `decision needed` fuer bewussten Alexander-Entscheidungsbedarf. Das Reibungslog bleibt eine sichere Notizvorlage und keine automatische Ticket-/Backlog-/QA-Plattform.
