...
- The later implementation must carry two versioned, tested repository templates: `platform-infra/docker-compose.phase3-catering-pilot.yml` and `edge-infra/docker-compose.phase3-catering-pilot.yml`. Their protected installed copies are exactly `/opt/catering-phase3/platform-compose.phase3.yml` and `/opt/catering-phase3/edge-compose.phase3.yml`; `/opt/catering-phase3/phase3.activation` is the only activation marker. Each copy/marker transaction is owner-/lock-scoped, staged/atomic, hash-checked, and recorded in the non-sensitive evidence manifest; general rsync/snapshot/restore paths must not touch that directory. A runtime `docker network connect` never recreates or restarts Edge, while later normal callers consume their mode-appropriate file set or fail closed.
...
The repository Phase-3 override is a versioned inert template, not an active source by itself. Under both exact host locks, the Catering owner stages it as `/opt/catering-phase3/platform-compose.phase3.yml`, atomically installs it, verifies the expected SHA-256 and `cmp`, and records prior/new hashes, lock identities, destination, and rollback artifact in the evidence manifest. The protected copy is outside all general Platform/Web rsync, snapshot, and restore trees; only the marker transaction may change it. Any failed hash, lock, source-chain, or protected-path check stops before live mutation.
...
- be synchronized/installed under both host locks in the fixed order atomically (staged file, exact hash/cmp verification, and a reversible prior-source snapshot), with source path, prior/new hashes, lock order, marker state, and rollback artifact recorded in the evidence manifest;
...
The durable activation state is one non-secret marker at the exact path `/opt/catering-phase3/phase3.activation`. The only protected installed artifacts are exactly:
...
The directory is outside every general Platform/Web/Edge `rsync --delete`, snapshot, and restore tree, owned by the existing deployment role with restrictive permissions, and covered by the baseline/rollback manifest. Only the marker/source transaction may change these three paths. Repository templates in `main` are inert and never activate Phase 3 by themselves.

The marker state machine is exact:

| State | Meaning and permitted callers |
| --- | --- |
| absent or `inactive` | Stable current Phase-2 Shared-Edge cutover state. Normal production callers use the complete three-file Platform Phase-2 chain and mode-appropriate Edge Base/Rehearsal chain; no caller may interpret this as pre-cutover or use Base+Production without edge-cutover. |
| `candidate` | Pilot is running under both host locks. Every normal mutating Platform, Web-recovery, Edge, cutover, rollback, and deploy caller stops fail closed. Only the pilot helper may continue its staged transaction. |
| `active` | Phase 3.1 is fully activated and the protected source copies/hash/network IDs are consistent. Normal callers may use the mode-dependent Phase-3 chains below after their own pre-mutation validation. |
| `rolling_back` | Restoration is running under both host locks. Every normal mutating caller stops fail closed until the exact prior absent/`inactive` state has been restored and read back. |

Absent and `inactive` are semantically identical for the current Phase-2 production state. They are not a permission to run a direct pre-cutover chain.
...
For `inactive`, the exact record is `schema=phase3.1`, `state=inactive`, `owner=catering-agents-platform`, both source hashes `absent`, and both network IDs `absent`; the record may instead be absent. No other fields are valid. Candidate resource IDs are `absent` only in the first candidate readback when the corresponding network was proven absent; after that network is created/verified, the candidate marker is atomically rewritten with its full ID before any subsequent mutation. `active` requires full 64-hex source hashes and network IDs. In `rolling_back`, every resource field may be `absent` or the full confirmed hash/ID from the last confirmed pilot state; these values are immutable rollback provenance, not a claim that the resource remains live until rollback ends. Container IDs are never configuration generations. The complete rendered Compose/config contract is validated separately and its hash is recorded in the evidence manifest, so a legitimate later recreate does not invalidate the marker merely by changing a container ID.

The marker contains no secret, environment value, authorization material, or file content. For `candidate` and `active`, any unknown state/field, duplicate field, malformed value, missing protected source, hash mismatch, network-ID mismatch, or active marker without complete rendered-config validation is `NO-GO` before any stop, recreate, sync, connect, or disconnect mutation; their source/network values must match the current live resources. For `rolling_back`, syntax, immutable provenance, and the baseline/rollback manifest remain strict, but current-live equality is deliberately not required while resources are being restored or removed. All normal mutating callers remain blocked solely by `state=rolling_back`; only the pilot helper may continue the manifest-checked rollback. A `candidate` marker is never a terminal GO; only successful active readback may precede `PILOT: GO`.

All Phase-3 activation, source synchronization, membership mutation, and rollback acquire `/opt/catering-agents-platform.deploy-lock` first and `/opt/shared-edge.deploy-lock` second, hold both across the complete marker/source/network transaction, and release in reverse order. Workflow concurrency is supplementary and cannot replace either host lock. The pilot sequence is fixed: (a) acquire both locks; (b) stage/install both protected sources atomically and verify exact hashes/cmp; (c) atomically write `candidate` and fully read it back; (d) only then perform the first network mutation; (e) complete the runtime migration and all evidence as a non-terminal `PILOT: GO CANDIDATE`; (f) atomically write `active` with full source hashes and network IDs and fully read it back; (g) run final invariants and smokes; (h) only then emit terminal `PILOT: GO`. No text or helper branch may emit terminal GO before successful active readback.

Rollback sequence is also fixed: (a) atomically derive and write `rolling_back` from the last confirmed pilot marker/provenance and fully read it back; (b) under both locks stage/restore the exact prior protected sources or proven absence; (c) additively restore every exact old Catering/Edge compatibility membership and alias; (d) run invariants and smokes; (e) remove new pilot memberships one at a time and only eligible run-created empty networks; (f) rerun invariants, smokes, source hashes, and manifest/provenance checks without requiring removed resources to remain live; (g) only then atomically restore the exact prior absent/`inactive` marker state and fully read it back; (h) only then emit terminal `PILOT: ROLLED BACK`. There is no transition directly from `active` to `inactive` at rollback start. Any failure leaves `rolling_back`, normal callers blocked, and terminal outcome `PILOT: NO-GO`.

General Platform/Web/Edge rsync, snapshot, and restore paths must exclude `/opt/catering-phase3` mechanically and must not copy, delete, or overwrite the protected files. Every normal mutating caller validates the marker, exact lock order, mode-specific file chain, source hashes, network IDs, and rendered-config hash before mutation: active plus missing/hash-false sources is `NO-GO`; inactive/absent plus templates is Phase-2-safe; candidate/rolling_back is always blocked. The pilot helper's `rolling_back` path instead validates marker syntax, immutable provenance, and the baseline/rollback manifest while resources are restored or removed.

The non-secret evidence manifest records marker state/path, prior and new marker hashes, both protected source hashes/paths, network IDs, rendered Compose/config hashes, lock order, mode, and rollback artifacts without secrets or protected file contents.
...
4b. **Activation and caller contract:** before any mutation, every normal Platform, web-listener, Edge, cutover, recovery, rollback, and relevant evidence caller validates the exact marker state, the two host locks in order `/opt/catering-agents-platform.deploy-lock` then `/opt/shared-edge.deploy-lock`, both protected source hashes, both network IDs, mode-specific file set, and rendered-config hash. Absent/`inactive` is the stable Phase-2 state; `candidate`/`rolling_back` is fail-closed for those normal callers; active with missing/hash-false sources is `NO-GO`. The marker becomes `candidate` before the first network mutation and `active` only after the non-terminal GO candidate; rollback starts from the last confirmed pilot provenance in `rolling_back` and restores the exact prior absent/`inactive` state only at the end. Protected files are outside and excluded from `rsync --delete`; no caller may stop/recreate/sync or mutate `.env` before these checks. The pilot helper alone may continue a `rolling_back` transaction under the manifest/provenance checks above.
...
11. **Protected state:** database/Caddy volumes, secret names/IDs, release metadata, Caddy/upstream text, DNS, TLS, image, and public port bindings stay unchanged. The edge-cutover removal of application host-port bindings remains required in every rendered Catering file chain. `/opt/catering-phase3/platform-compose.phase3.yml`, `/opt/catering-phase3/edge-compose.phase3.yml`, and `/opt/catering-phase3/phase3.activation` are protected from every general rsync/snapshot/restore path; only the locked marker transaction may change them.
12. **Safe rollback order:** under both exact host locks, write/read `rolling_back`; restore and hash/cmp-verify prior protected sources/absence; restore old Catering/Edge compatibility memberships and aliases; run invariants/smokes; remove new pilot memberships one by one and only eligible run-created empty networks; rerun invariants/smokes/source checks; restore/read the exact prior absent/`inactive` marker; only then emit `PILOT: ROLLED BACK`. Any failure leaves `rolling_back`, blocks normal callers, and emits `PILOT: NO-GO`. Existing networks remain, and Edge is not unnecessarily interrupted.
13. **Evidence:** emit only non-sensitive Phase-3 manifests from the separate pilot helper with marker state/path, protected source hashes/paths, network IDs/members, provenance, rendered Compose/config hashes, upstreams, smokes, rollback point, lock order, and exact mutation steps. The existing Phase-2 evidence helper never evaluates these states. Terminal Phase-3 statuses are exactly `PILOT: GO`, `PILOT: ROLLED BACK`, or `PILOT: NO-GO`; `PILOT: GO CANDIDATE` is non-terminal and may never be reported as GO.
...
- [ ] Add the non-secret `/opt/catering-phase3/phase3.activation` marker contract and exact protected copies `/opt/catering-phase3/platform-compose.phase3.yml` and `/opt/catering-phase3/edge-compose.phase3.yml`. The RED/GREEN test must prove absent/inactive is stable Phase-2, candidate/rolling_back block normal mutators, active binds exact source hashes and network IDs, marker/source installation is staged and atomic, `rsync --delete` cannot remove any protected artifact, and rollback restores the exact prior marker/source state before membership removal. Inject failures before the first network creation, between the first and second network creations, and after both creations, plus the exact pre-existing-resource case; assert candidate progress and rolling_back provenance/manifest handling in each case, including that pre-existing networks remain untouched.
...
- [ ] Acquire both exact host locks in fixed order, atomically install/hash-check the two protected source copies, write/read `candidate` before the first network mutation, and validate the mode-specific complete file sets. Every later canonical recreate/deploy/recovery/rollback path must consume its state-appropriate file set or fail closed; source installation itself must not recreate or restart any container.
...
- [ ] After all pilot gates, smokes, foreign invariants, isolation proofs, source persistence/recreate checks, and evidence are green, write/read `active` binding both override hashes and full network IDs, then rerun final invariants/smokes and only then emit terminal `PILOT: GO`. Until successful active readback, only `PILOT: GO CANDIDATE` is permitted; on failure, transition to `rolling_back`, never directly to inactive.
- [ ] On any failure, write/read `rolling_back`, restore both prior protected source artifacts/absence, additively re-establish every exact old Catering/Edge membership and alias, pass invariants/smokes, remove new memberships one at a time, and remove only networks marked `created-by-run` after exact ID/labels/parameters, emptiness, and no-foreign-consumer checks. Restore/read the exact prior absent/`inactive` marker only at the end. Pre-existing networks remain. Emit `PILOT: ROLLED BACK` only after that final readback; any failure leaves `rolling_back` and emits `PILOT: NO-GO`.
...
- [ ] After successful Catering migration, complete the non-sensitive evidence manifest, independent review, workflow CI, and operational acceptance.
...
7. Read and validate `/opt/catering-phase3/phase3.activation` under `/opt/catering-agents-platform.deploy-lock` then `/opt/shared-edge.deploy-lock` before every mutation and after rollback. Verify the marker's exact state/fields, source hashes, and applicable network IDs; validate mode and rendered-config hash separately from the execution context and evidence manifest because they are not marker fields. Test candidate/rolling_back blocking for normal callers, rolling_back provenance/manifest validation without a live-resource requirement, active hash/source failures, exact final active readback, and inactive templates not activating Phase 3.
...
- The exact non-secret marker automaton (`absent`/`inactive`/`candidate`/`active`/`rolling_back`), shared lock order, and caller/workflow allowlist make templates inert until active readback and keep legacy deploys safe before activation and after rollback; state/source/hash drift fails closed before mutation.
- Both Catering and Shared Edge have durable, versioned pilot source contracts with protected copies and mode-specific chains; each source/marker install/rollback is lock-scoped, atomic, hash-checked, and manifest-bound, while runtime connect remains non-recreating and later normal recreate consumes the correct chain. Only the authorized external `catering_ingress` membership changes; Caddy, upstreams, hostnames, ports, DNS, TLS, image, restart, and runtime behavior remain unchanged.
- Rollback enters `rolling_back`, restores marker/sources and old memberships before removing new memberships, and removes only run-created, empty, exact-label/ID-verified Catering networks; failure never falsely returns to inactive.
...
- The current turn changes only this plan file and performs no runtime or Git mutation.
