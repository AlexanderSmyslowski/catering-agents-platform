...
interface ConfirmBody extends PreviewBody {
  previewId?: string;
}
...
function previewIdFor(
  runtime: QuantityWorkflowRuntimeComponent,
  edit: SupportedEdit,
  preview: ProductionQuantityOverridePreview
): string {
...
function findRuntime(
  runtimes: QuantityWorkflowRuntimeComponent[],
  componentId: string
): QuantityWorkflowRuntimeComponent | undefined {
...
function validEdit(value: unknown): value is SupportedEdit {
  ...
  if (edit.origin === "target_output") {
    return typeof edit.perUnitAmount === "number" && Number.isFinite(edit.perUnitAmount) &&
      typeof edit.unit === "string" && edit.unit.trim().length > 0;
  }
  if (edit.origin === "purchase_ingredient") {
    return typeof edit.ingredientId === "string" && edit.ingredientId.trim().length > 0 &&
      typeof edit.amount === "number" && Number.isFinite(edit.amount) &&
      typeof edit.unit === "string" && edit.unit.trim().length > 0;
  }
  return false;
}

export function registerProductionQuantityWorkflowRoutes(
  app: FastifyInstance,
  deps: ProductionQuantityWorkflowRouteDependencies
): void {
  const {
    auditLog,
    trustedActorSecret,
  ...
  app.get<{ Params: { caseId: string } }>(
    "/v1/production/cases/:caseId/quantity-workflow",
    async (request, reply) => {
      ...
      return reply.send({
        items: runtimes.map((runtime) => ({
          ...buildQuantityWorkflowProjection(runtime.projectionInput),
  ...
  app.post<{
    Params: { caseId: string; componentId: string };
    Body: PreviewBody;
  }>(
    ...
    async (request, reply) => {
      const forbidden = requireProductionOperator(request, reply, trustedActorSecret, allowDevActorHeader);
      if (forbidden) return forbidden;
      if (!validEdit(request.body?.edit)) {
        return reply.code(400).send({ error: "quantity_edit_invalid" });
      }
      const actor = actorForRequest(request, trustedActorSecret, allowDevActorHeader);
      ...
      return reply.send({
        previewId: previewIdFor(runtime, request.body.edit, preview),
        sourceRevision: runtime.revision,
        preview
      });
    }
  ...
  app.post<{
    ...
    async (request, reply) => {
      const forbidden = requireProductionOperator(request, reply, trustedActorSecret, allowDevActorHeader);
      if (forbidden) return forbidden;
      if (!validEdit(request.body?.edit) || typeof request.body?.previewId !== "string") {
        return reply.code(400).send({ error: "quantity_confirmation_invalid" });
      }
      const actor = actorForRequest(request, trustedActorSecret, allowDevActorHeader);
      const runtime = findRuntime(await resolveRuntime(actor, request.params.caseId), request.params.componentId);
      if (!runtime) return reply.code(404).send({ error: "quantity_component_not_found" });
      ...
      if (preview.status !== "preview_ready" || request.body.previewId !== expectedPreviewId) {
        return reply.code(409).send({ error: "quantity_preview_stale" });
      }

      const confirmedAt = new Date().toISOString();
      const overrideId = `quantity-override-${createHash("sha256")
        .update(`${expectedPreviewId}\0${actor.businessId}\0${actor.name}`)
        .digest("hex")}`;
      const confirmation = confirmQuantityOverride({
        preview: preview.corePreview,
        overrideId,
        confirmedAt,
        operatorId: actor.name
      });
      if (confirmation.status !== "confirmed") {
        return reply.code(409).send({ error: "quantity_confirmation_blocked", issues: confirmation.issues });
      }
      ...
      const status = recalculation.bridge.status === "ready_for_scaling" && recalculation.quantityDecision.decision.reviewStatus === "approved"
        ? "regenerated"
        : "review_required";
      return reply.send({
        status,
